...
fte_theme <- function(font_family = "serif",
  ...
  if(legend_title_on){
    ...
    legend_text <- element_text(colour = color.axis.title,
                               size = rel(1.5))
  }
...
label_cuts <- function(type){
  ...
  function(x){
    ...
    if (type == "dollar") {
      output <- str_c(alternate_dollar(start_points + start_type), "-",
      ...
      output <- str_c(scales::percent(start_points + 0.01 * start_type,
                                      accuracy = 1,
                                      suffix = ""), "-",
